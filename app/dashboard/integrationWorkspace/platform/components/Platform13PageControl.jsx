...
const BRAND_HOVER = "var(--brand-secondary)";
...
export function Input({ label, required, placeholder, value, type = "text", icon, suffix, hint, onChange, className = "", ...props }) {
  return (
    <div>
      ...
      <FieldWrap icon={icon} suffix={suffix}>
        <input
          ...
          onChange={onChange}
          {...props}
          className={`w-full rounded-lg border border-[#d9e2f0] bg-[#fbfcff] px-3 py-2.5 text-sm text-gray-800 outline-none transition-all placeholder:text-gray-300 hover:border-[#b8c9e4] focus:border-[var(--brand-primary)] focus:ring-2 focus:ring-[var(--brand-primary)]/15 ${className}`}
        />
      </FieldWrap>
...
export function Select({ label, required, placeholder, options = [], value, onChange, icon, hint, disabled = false }) {
    ...
    <div>
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-y-0 right-0 flex w-10 items-center justify-center border-l border-[#e7edf6] bg-[linear-gradient(180deg,#fbfdff_0%,#f3f7fd_100%)]">
          <ChevronDown className={`h-3.5 w-3.5 transition-colors ${hasValue ? "text-[var(--brand-primary)]" : "text-gray-400"}`} />
        </div>
      </div>
      {hint && <p className="mt-1 text-[11px] text-gray-400">{hint}</p>}
    </div>
...
export function MultiSelect({ label, required, placeholder, options = [], value = [], onChange, hint, disabled = false }) {
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      <button
        ...
        className="flex min-h-[52px] w-full items-start justify-between gap-3 rounded-lg border border-[#d9e2f0] bg-[#fbfcff] px-3.5 py-3 text-left text-sm text-gray-800 shadow-sm transition-all hover:border-[color:var(--brand-primary)] focus:border-[color:var(--brand-primary)] focus:ring-2 focus:ring-[var(--brand-primary)]/15 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-400"
        disabled={disabled}
        style={{ ["--brand-primary"]: BRAND_PRIMARY }}
      >
        <div className="flex flex-1 flex-wrap gap-1.5">
          {value.length ? (
            <span className="pt-0.5" style={{ color: BRAND_PRIMARY }}>
        ...
        <div className="mt-2 rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
          <div className="mb-2 flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={handleClearAll}
              className="text-[11px] font-semibold text-gray-500 transition-colors hover:text-[var(--brand-secondary)]"
            >
              Clear all
            </button>
          </div>
