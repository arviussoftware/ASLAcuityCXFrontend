...
import { ChevronLeft, Search, ChevronDown, ChevronRight, Shield, Building2, Save } from "lucide-react";
...
const ACTION_PRIV_IDS = [ANNOTATION_PRIV.CREATE, ANNOTATION_PRIV.EDIT, ANNOTATION_PRIV.DELETE];

// ─── Pure helpers ─────────────────────────────────────────────────────────────
const isIntegrationModule = (name = "") => String(name).toLowerCase().includes("integration");
const getPrivilegeId = (p) => Number(p?.PrivilegeId ?? p?.privilegeId ?? p?.id ?? p?.ID);
const getPrivilegeName = (p) =>
  p?.PrivilegeName ?? p?.privilegeName ?? p?.privilegename ??
  p?.privilege_name ?? p?.privilege_label ?? p?.label ??
  p?.name ?? p?.Name ?? p?.title ?? "";

const getDefaultPrivilegeIds = (moduleId, privileges = []) =>
  privileges.map(getPrivilegeId).filter(
    (id) =>
      id !== NONE_PRIVILEGE_ID &&
      !(Number(moduleId) === DASHBOARD_MODULE_ID && id === AGENT_DASHBOARD_PRIV_ID)
  );

const buildPrivilegeSelectionState = (moduleId, privileges = [], selectedIds = []) => {
...
const selectAllSources = (sources = []) => sources.map((s) => ({ ...s, selected: true }));

const collectDescendantIds = (node) => {
  if (!node) return [];
  return [String(node.id), ...(node.children || []).flatMap((c) => collectDescendantIds(c))];
};
...
function PrivilegeChip({ label, selected, disabled, onChange }) {
  return (
    <label className={`flex items-center gap-2 w-full px-3 py-2 rounded-lg border text-xs font-medium text-left transition-all duration-150 select-none ${disabled ? "opacity-40 cursor-not-allowed bg-gray-50 border-gray-200 text-gray-400" : "bg-white border-gray-200 text-gray-600 hover:border-gray-300 hover:text-gray-900"}`}>
      <input type="checkbox" className="h-3.5 w-3.5 accent-[var(--brand-primary)]" checked={selected} disabled={disabled} onChange={onChange} />
...
function OrgNode({ node, depth, selectedOrgIds, onToggle, onActivate }) {
  ...
  const label = node.label || node.name || node.OrgName || node.Name || "Untitled";
  return (
    <li>
      <div className="flex items-center gap-0.5">
        <button type="button" onClick={() => hasChildren && setOpen((v) => !v)}
        ...
        <label className="flex-1 min-w-0 flex items-center gap-2 px-2 py-1.5 rounded-md">
          <input type="checkbox" className="h-3.5 w-3.5 accent-[var(--brand-primary)]" checked={isSelected} onChange={() => onToggle(node.id)} />
          <button type="button" onClick={() => onActivate(node.id)}
      ...
      {hasChildren && open && (
        <ul className="mt-0.5 ml-5 pl-2.5 border-l border-gray-100 space-y-0.5">
          {node.children.map((child) => (
            <OrgNode key={child.id} node={child} depth={depth + 1} selectedOrgIds={selectedOrgIds} onToggle={onToggle} onActivate={onActivate} />
          ))}
        </ul>
      )}
    </li>
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#F8F9FA]">
      <div className="flex flex-col items-center gap-4 p-8 rounded-3xl bg-white border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
        <div className="relative flex items-center justify-center">
          <div className="w-8 h-8 rounded-full animate-spin" style={{ border: "2.5px solid #E2E8F0", borderTopColor: "var(--brand-primary)" }} />
          <div className="absolute w-12 h-12 rounded-full border border-[var(--brand-primary)]/20 animate-ping opacity-75" />
        </div>
        <span className="text-xs font-semibold text-gray-600 tracking-tight">Checking access...</span>
      </div>
    </div>
  );
}
...
export default function RoleModulesPage({ params }) {
  ...
  const resolvedRoleId = useMemo(() => {
    ...
    if (directId && !INVALID.includes(String(directId))) return String(directId);
  ...
  useEffect(() => { roleIdRef.current = resolvedRoleId; }, [resolvedRoleId]);
  ...
  useEffect(() => { persistedOrgIdsRef.current = persistedOrgIds; }, [persistedOrgIds]);
  ...
  useEffect(() => {
    const bootstrap = async () => {
      try {
        ...
        if (!meRes.ok) { setHasAccess(false); router.replace("/not-found"); return; }
        ...
        if (!superAdmin && Number(roleIdRef.current) === 1) { router.replace("/not-found"); return; }
        ...
        const ok = privRes.ok && (privData.PrivilegeList || []).some((p) => Number(p.PrivilegeId ?? p.privilegeId) === 12);
        setHasAccess(ok);
        if (!ok) { router.replace("/not-found"); return; }

        if (treeRes.ok) {
          const treeData = await treeRes.json();
          const normalize = (node) => ({
            id: node.id ?? node.ID ?? node.orgId ?? node.OrgId ?? node.organizationId,
            label:
              node.Name || node.name || node.OrgName || node.orgName ||
              node.OrganizationName || node.organizationName || node.label || "Untitled",
            children: (node.children || []).map(normalize),
          });
          const roots = Array.isArray(treeData.organizations) ? treeData.organizations.map(normalize) : [];
          setOrgTree(roots);
        }
      } catch {
        setHasAccess(false);
  ...
  const loadBundle = useCallback(async (orgId, knownPersistedIds = []) => {
    ...
    try {
      ...
      const allPersistedIds = [...new Set([...persistedOrgIdsRef.current, ...knownPersistedIds])];
      ...
      bundle.modulePayloads.forEach((p) => {
        ...
        const effectiveIds = savedIds.length === 0 && !isPersistedOrg ? defaults : savedIds;
        const hasSaved = effectiveIds.some((id) => id !== NONE_PRIVILEGE_ID);

        nextModules[moduleId] = { checked: hasSaved, name: moduleName };
        nextPrivileges[moduleId] = buildPrivilegeSelectionState(moduleId, p.privileges || [], effectiveIds);
        ...
        if (isIntegrationModule(moduleName)) {
          ...
          nextSources[moduleId] = {
            ...
            sources: (p.sources || []).map((s) => {
              ...
              return {
                id: sid,
                label: s.source || s.Source || s.Name || s.label || `Source ${sid}`,
  ...
  useEffect(() => {
    ...
    const preload = async () => {
      ...
      try {
        const res = await fetch(`/api/roleManagement/savedOrgs/${resolvedRoleId}`, {
          method: "GET",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${process.env.NEXT_PUBLIC_API_TOKEN}` },
          credentials: "include", cache: "no-store",
        });
        ...
        if (res.ok) {
          ...
          const detectedOrgIds = (data.orgIds || []).map(String).filter((id) => treeIdSet.has(id));
  ...
  useEffect(() => {
    if (selectedOrgIds.length === 0) { setActiveOrgId(""); return; }
  ...
  useEffect(() => {
    ...
    setSelectedOrgNames(selectedOrgIds.map((id) => findName(orgTree, id)).filter(Boolean));
  }, [selectedOrgIds, orgTree]);

  // ── Org toggle ─────────────────────────────────────────────────────────────
  const handleOrgToggle = useCallback((orgId) => {
  ...
  const handleOrgActivate = useCallback((orgId) => {
    const id = String(orgId);
    if (!selectedOrgIds.includes(id)) { handleOrgToggle(id); return; }
    setActiveOrgId(id);
    setSelectedModules({});
    setModulePrivileges({});
    setModuleSourceAccess({});
  ...
  // ── Module toggle ──────────────────────────────────────────────────────────
  const handleCheckboxChange = useCallback(async (moduleId, moduleName) => {
    try {
      ...
      if (!current.length) {
        ...
        const data = await res.json();
        if (!res.ok) return;
        const ids = (data.savedPrivileges || []).map((p) => getPrivilegeId(p));
        const defaults = getDefaultPrivilegeIds(moduleId, data.privileges || []);
        const effective = ids.length === 0 ? defaults : ids;
        setModulePrivileges((prev) => ({ ...prev, [moduleId]: buildPrivilegeSelectionState(moduleId, data.privileges, effective) }));
        return;
      }
      ...
      setModulePrivileges((prev) => ({ ...prev, [moduleId]: buildPrivilegeSelectionState(moduleId, prev[moduleId] || [], defaults) }));

      if (isIntegrationModule(moduleName)) {
        setModuleSourceAccess((prev) => {
          const sources = prev[moduleId]?.sources || [];
          if (!sources.length) return prev;
          return { ...prev, [moduleId]: { loading: false, sources: selectAllSources(sources) } };
        });
      }
    } catch (err) {
  ...
  const handlePrivilegeChange = useCallback((moduleId, privilegeId) => {
    setModulePrivileges((prev) => {
      ...
      let updated = current.map((p) => {
        ...
        if (getPrivilegeId(p) === NONE_PRIVILEGE_ID && p.selected) return { ...p, selected: false };
        if (getPrivilegeId(p) === privilegeId) return { ...p, selected: !p.selected };
        return p;
      });

      if (Number(moduleId) === INTERACTION_MODULE_ID) {
        if (privilegeId === ANNOTATION_PARENT_ID) {
          const parentOn = updated.find((p) => getPrivilegeId(p) === ANNOTATION_PARENT_ID)?.selected ?? false;
          updated = updated.map((p) =>
            ANNOTATION_SUB_IDS.includes(getPrivilegeId(p)) ? { ...p, selected: parentOn } : p
          );
        }
        if (ACTION_PRIV_IDS.includes(privilegeId)) {
          const nowOn = updated.find((p) => getPrivilegeId(p) === privilegeId)?.selected ?? false;
          if (nowOn)
            updated = updated.map((p) =>
              getPrivilegeId(p) === ANNOTATION_PRIV.VIEW ? { ...p, selected: true } : p
            );
        }
        const anyAction = updated.some((p) => ACTION_PRIV_IDS.includes(getPrivilegeId(p)) && p.selected);
        if (privilegeId === ANNOTATION_PRIV.VIEW && anyAction)
          updated = updated.map((p) =>
            getPrivilegeId(p) === ANNOTATION_PRIV.VIEW ? { ...p, selected: true } : p
          );
        updated = updated.map((p) =>
          getPrivilegeId(p) === ANNOTATION_PRIV.VIEW ? { ...p, disabled: anyAction } : p
        );
        if (ANNOTATION_SUB_IDS.includes(privilegeId)) {
          const anySub = updated.some((p) => ANNOTATION_SUB_IDS.includes(getPrivilegeId(p)) && p.selected);
          if (!anySub)
            updated = updated.map((p) =>
              getPrivilegeId(p) === ANNOTATION_PARENT_ID ? { ...p, selected: false } : p
            );
        }
      }

      const noneOn = updated.find((p) => getPrivilegeId(p) === NONE_PRIVILEGE_ID)?.selected ?? false;
      updated = updated.map((p) => ({ ...p, disabled: getPrivilegeId(p) !== NONE_PRIVILEGE_ID && noneOn }));
    ...
    if (privilegeId === NONE_PRIVILEGE_ID) {
      setModuleSourceAccess((prev) => ({
        ...prev,
        [moduleId]: { loading: false, sources: (prev[moduleId]?.sources || []).map((s) => ({ ...s, selected: false })) },
      }));
    }
  }, []);
    ...
    setModuleSourceAccess((prev) => {
      ...
      setModulePrivileges((pp) => ({
        ...
        [moduleId]: (pp[moduleId] || []).map((p) => {
          if (getPrivilegeId(p) === NONE_PRIVILEGE_ID) return { ...p, selected: false, disabled: false };
          if (getPrivilegeId(p) === VIEW_PRIVILEGE_ID)
            return anySelected ? { ...p, selected: true, disabled: false } : { ...p, disabled: false };
  ...
  const handleSubmit = async () => {
    const roleId = roleIdRef.current;
    if (!roleId) { alert("Role ID is missing. Please refresh and try again."); return; }

    const orgIds = selectedOrgIds.filter(Boolean);
    if (!orgIds.length) { alert("Please select at least one organization."); return; }

    const checkedIds = Object.keys(selectedModules).filter((id) => selectedModules[id].checked);
    const uncheckedIds = Object.keys(selectedModules).filter((id) => !selectedModules[id].checked);

    let userId = null, userName = null;
    try {
      const meRes = await fetch("/api/auth/me", { method: "GET", credentials: "include", cache: "no-store" });
      if (meRes.ok) { const me = await meRes.json(); userId = me?.userId ?? null; userName = me?.userFullName ?? null; }
    } catch { /* ignore */ }
    ...
    for (const moduleId of checkedIds) {
      ...
      const selSources = (moduleSourceAccess[moduleId]?.sources || []).filter((s) => s.selected);

      if (parseInt(moduleId) === 5 && privs.some((p) => getPrivilegeId(p) === 16) && !privs.some((p) => getPrivilegeId(p) === 10)) {
        alert('Please select "Create/Edit" privilege for module Form Designer.'); return;
      }
      if (parseInt(moduleId) === INTERACTION_MODULE_ID) {
        const parentOn = privs.some((p) => getPrivilegeId(p) === ANNOTATION_PARENT_ID);
        const subOn = privs.some((p) => ANNOTATION_SUB_IDS.includes(getPrivilegeId(p)));
        if (parentOn && !subOn) { alert("Please select at least one Annotation permission."); return; }
      }
      if (!privs.length) { alert(`Select at least one privilege for: ${modName}`); return; }
      if (isInteg && !noneOn && !selSources.length) { alert(`Select at least one source for: ${modName}`); return; }

      orgIds.forEach((orgId) => {
        privByOrg[orgId] = privByOrg[orgId] || [];
        sourceByOrg[orgId] = sourceByOrg[orgId] || [];
        privs.forEach((p) => {
          privByOrg[orgId].push({ roleid: roleId, moduleId, privilegeId: getPrivilegeId(p), orgId, userId });
          hasAny = true;
        });
        if (isInteg && !noneOn) selSources.forEach((s) => sourceByOrg[orgId].push({ roleid: roleId, moduleId, sourceId: s.id, orgId, userId }));
      });
    }
      ...
      for (const orgId of orgIds) {
        const res = await fetch("/api/roleManagement/save", {
          ...
          body: JSON.stringify({
            ...
            roleid: roleId, roleName, userId, userName, orgId,
          }),
        });
  ...
  const selectedOrgOptions = selectedOrgIds.map((id) => ({ id, label: findName(orgTree, id) || id })).filter((o) => o.label);
  ...
  const noModules = !noOrgSelected && modules.length === 0 && !moduleDataLoading;
  const showInitialLoader = initialOrgLoading && selectedOrgIds.length === 0;
  const showOverlay = initialOrgLoading || moduleDataLoading;
  const displayOrgLabel = selectedOrgNames.length > 1 ? `${selectedOrgNames.length} organizations` : selectedOrgNames[0] || "";
  const overlayTitle = moduleDataLoading
    ? "Loading modules and privileges"
    : !roleName ? "Preparing role privileges" : "Loading privileges";
  ...
  if (notFound) return (
    <NotFound message="The role you are trying to assign privileges to does not exist." redirectUrl="/dashboard/roleManagement" redirectText="Go Back" />
  );
          ...
          <div className="max-w-screen-xl mx-auto px-6 pt-2 pb-2">
            <div className="bg-white border border-gray-200 rounded-2xl px-5 py-2.5 shadow-sm">
              <div className="flex items-center gap-4">
                <button onClick={() => router.push("/dashboard/roleManagement")}
                ...
                <div className="ml-auto flex items-center gap-2">
                  ...
                  <button onClick={() => { setSelectedModules({}); setModulePrivileges({}); setModuleSourceAccess({}); }} className="btn-ghost">
                    Clear
                  </button>
        ...
        <main className="max-w-screen-xl mx-auto px-6 py-3">
          <div className="relative">
            <div
              ...
              {/* LEFT SIDEBAR */}
              <aside className="bg-white border border-gray-200 rounded-2xl overflow-visible sticky top-[48px] flex flex-col shadow-sm" style={{ height: "calc(100vh - 120px)" }}>
                <div className="px-4 pt-4 pb-3 border-b border-gray-100 flex-shrink-0">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: "#F3F4F6", color: "var(--brand-primary)" }}>
                      {orgCount}
                    </span>
                  </div>
                  <div className="relative">
                ...
                {selectedOrgOptions.length > 0 && (
                  <div className="px-4 py-2.5 border-b border-gray-100 bg-gray-50/70 flex-shrink-0">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <button onClick={() => { setSelectedOrgIds([]); setSelectedOrgNames([]); setActiveOrgId(""); setSelectedModules({}); setModulePrivileges({}); setModuleSourceAccess({}); setCollapsedModules({}); }}
                    ...
                    <div className="flex flex-wrap items-start gap-1">
                      {visibleOrgOptions.map((org) => {
                        ...
                        return (
                          <div key={org.id} className="flex items-center gap-1.5">
                            <input type="checkbox" className="h-3.5 w-3.5 accent-[var(--brand-primary)]" checked
                              onChange={() => handleOrgToggle(org.id)} />
                            <button type="button" onClick={() => handleOrgActivate(org.id)}
                              className="text-[10px] font-semibold px-2 py-0.5 rounded-full border transition-all"
                              style={isActive ? { background: "var(--brand-primary)", borderColor: "var(--brand-primary)", color: "#fff" } : { background: "#fff", borderColor: "#D1D5DB", color: "var(--brand-primary)" }}
                      ...
                      {hiddenOrgOptions.length > 0 && (
                        <div className="relative basis-full"
                          ...
                          <button type="button" onClick={() => setShowOrgOverflow((v) => !v)}
                  ...
                  <div className="bg-white border border-gray-200 rounded-2xl px-5 py-3 flex items-center gap-4 shadow-sm">
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-gray-900 truncate">
                        {showInitialLoader ? "Loading organizations" : noOrgSelected ? "No organization selected" : displayOrgLabel}
                      </p>
                      ...
                      {showInitialLoader && (
                        <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
                          <div className="relative flex items-center justify-center mb-4">
                            <div className="w-8 h-8 rounded-full animate-spin" style={{ border: "2.5px solid #E5E7EB", borderTopColor: "var(--brand-primary)" }} />
                            <div className="absolute w-12 h-12 rounded-full border border-[var(--brand-primary)]/20 animate-ping opacity-75" />
                          </div>
                          <p className="text-xs font-bold text-gray-800 tracking-tight">Loading organizations</p>
                          <p className="text-[11px] text-gray-400 mt-1.5 max-w-[240px] leading-relaxed">
                            Checking which organizations already have saved privileges for this role.
                          </p>
                        </div>
                      )}
                      {noOrgSelected && !showInitialLoader && (
                        <EmptyState icon={Building2} title="Select an organization" message="Choose one or more organizations from the sidebar to load role privileges." />
                      )}
                      {noModules && (
                        <EmptyState icon={Shield} title="No modules found" message="No modules are configured for this role." />
                      )}
                      ...
                      {showModules && (
                        <ul className="divide-y divide-gray-100">
                          {modules.map((module) => {
                            ...
                            const sourceState = moduleSourceAccess[module.ID] || { loading: false, sources: [] };
                            const countable = privileges.filter((p) => getPrivilegeId(p) !== ANNOTATION_PARENT_ID);
                            const selCount = countable.filter((p) => p.selected).length;
                            const srcCount = (sourceState.sources || []).filter((s) => s.selected).length;

                            return (
                              <li key={module.ID}>
                                <div className={`flex items-center gap-4 px-5 py-4 transition-colors ${isChecked ? "bg-white" : "bg-white hover:bg-gray-50/50"}`}>
                                  <ModuleToggle checked={isChecked} onChange={() => handleCheckboxChange(module.ID, module.ModuleName)} />
                                  <div className="flex-1 min-w-0">
                                    <p className={`text-sm font-semibold truncate transition-colors ${isChecked ? "text-gray-900" : "text-gray-400"}`}>
                                      {module.ModuleName}
                                    </p>
                                    {isChecked && privileges.length > 0 && (
                                      <p className="text-[10px] mt-0.5" style={{ color: "var(--brand-primary)" }}>
                                        {selCount} of {countable.length} privilege{countable.length !== 1 ? "s" : ""} enabled
                                      </p>
                                    )}
                                    {isChecked && isIntegrationModule(module.ModuleName) && (
                                      <p className="text-[10px] mt-0.5 text-gray-500">{srcCount} source{srcCount !== 1 ? "s" : ""} selected</p>
                                    )}
                                  </div>
                                  {isChecked && privileges.length > 0 && (
                                    <button type="button"
                                ...
                                {isChecked && !isCollapsed && (privileges.length > 0 || isIntegrationModule(module.ModuleName)) && (
                                  <div className="px-5 pb-4 pt-3 bg-gray-50/60 border-t border-gray-100">
                                    {privileges.length > 0 && (() => {
                                      const mainPrivs = privileges.filter((p) => !ANNOTATION_SUB_IDS.includes(getPrivilegeId(p)));
                                      const subPrivs = privileges.filter((p) => ANNOTATION_SUB_IDS.includes(getPrivilegeId(p)));
                                      const annParent = privileges.find((p) => getPrivilegeId(p) === ANNOTATION_PARENT_ID);
                                      const showAnnotation = Number(module.ID) === INTERACTION_MODULE_ID && subPrivs.length > 0;
                                      return (
                                        <>
                                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                                            {mainPrivs.map((priv) => (
                                              <PrivilegeChip key={getPrivilegeId(priv)}
                                                label={getPrivilegeName(priv) || priv.privilege || priv.Privilege || priv.label || priv.title || ""}
                                                selected={priv.selected} disabled={priv.disabled}
                                                onChange={() => handlePrivilegeChange(module.ID, getPrivilegeId(priv))} />
                                            ))}
                                          </div>
                                          {/* Annotation Permissions — hidden (feature disabled in UI) */}
                                          {false && showAnnotation && (
                                            <div className={`mt-3 rounded-xl border bg-white overflow-hidden transition-opacity duration-150 ${annParent?.selected ? "border-[var(--brand-primary)]/20 opacity-100" : "border-gray-200 opacity-40 pointer-events-none"}`}>
                                              ...
                                              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 p-3">
                                                {subPrivs.map((priv) => (
                                                  <PrivilegeChip key={getPrivilegeId(priv)}
                                                    ...
                                                    disabled={priv.disabled || !annParent?.selected}
                                                    onChange={() => handlePrivilegeChange(module.ID, getPrivilegeId(priv))} />
                                                ))}
                                              </div>
                                            </div>
                                    ...
                                    {isIntegrationModule(module.ModuleName) && (
                                      <div className="mt-3 pt-3 border-t border-gray-200">
                                        ...
                                        {sourceState.loading ? (
                                          <p className="text-[11px] text-gray-400">Loading sources...</p>
                                        ) : (
                                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                                            {sourceState.sources.map((source) => {
                                              const noneOn = (modulePrivileges[module.ID] || []).find((p) => getPrivilegeId(p) === NONE_PRIVILEGE_ID)?.selected;
                                              return (
                                                <PrivilegeChip key={source.id} label={source.label}
                                                  selected={source.selected} disabled={!!noneOn}
                                                  onChange={() => handleSourceToggle(module.ID, source.id)} />
                                              );
                                            })}
                                          </div>
                                        )}
            ...
            {showOverlay && (
              <div className="absolute inset-0 z-20 flex items-center justify-center rounded-2xl bg-slate-900/5 backdrop-blur-[3px] transition-all duration-300">
                <div className="flex flex-col items-center gap-4 rounded-3xl border border-white/45 bg-white/80 p-8 shadow-[0_20px_50px_rgba(0,0,0,0.08)] backdrop-blur-md max-w-sm text-center">
                  ...
                  <div>
                    <h3 className="text-[13px] font-bold text-gray-800 tracking-tight">{overlayTitle}</h3>
                    <p className="text-[11px] text-gray-500 mt-1.5 leading-relaxed">{overlayMessage}</p>
                  </div>
                </div>
              </div>
            )}
