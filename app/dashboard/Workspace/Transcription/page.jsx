...
import {
  CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight,
  Database, Edit2, Info, Layers, Loader2, Mic2, Plus,
  RefreshCw, Search, Settings2, ToggleLeft, ToggleRight, Cpu, X,
} from "lucide-react";
import {
  Field, Section, SelectControl, TextControl, Toggle,
...
const PRIORITY_META = {
  ...
  above_normal: { color: "text-amber-500", bg: "bg-amber-500/10", border: "border-amber-500/30" },
  normal: { color: "text-emerald-500", bg: "bg-emerald-500/10", border: "border-emerald-500/30" },
  below_normal: { color: "text-blue-500", bg: "bg-blue-500/10", border: "border-blue-500/30" },
  low: { color: "text-violet-500", bg: "bg-violet-500/10", border: "border-violet-500/30" },
};
...
const getPriorityLabel = (v) =>
  PRIORITY_OPTIONS.find((o) => o.value === String(v || "").toLowerCase())?.label ?? v ?? "—";
...
function PriorityBadge({ priority }) {
  ...
  const m = PRIORITY_META[key] ?? { color: "text-gray-500", bg: "bg-gray-500/10", border: "border-gray-500/30" };
  return (
...
function Pagination({ totalItems, pageSize, pageNumber, onPage, onSize }) {
  ...
  const pages = useMemo(() => {
    ...
    const set = new Set([1, totalPages, pageNumber - 1, pageNumber, pageNumber + 1].filter((p) => p >= 1 && p <= totalPages));
      ...
      <form onSubmit={onSubmit} className="flex flex-col gap-4">
        ...
        >
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Field label="Rule Name" required hint="e.g. Compliance_Transcription_Rule">
              <TextControl value={form.ruleName} onChange={update("ruleName")} placeholder="Compliance_Transcription_Rule" />
            </Field>

            <Field
              label="Transcription Engine" required
            ...
            >
              <SelectControl
                ...
                icon={<Cpu size={15} />}
              >
                <option value="">{providersLoading ? "Loading…" : "— Select Engine —"}</option>
                {providers.map((p) => (
            ...
            <Field label="Processing Priority" hint="Determines queue priority for transcription jobs.">
              <SelectControl value={form.processingPriority} onChange={update("processingPriority")} icon={<Layers size={15} />}>
                {PRIORITY_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </SelectControl>
            </Field>

            <Field label="Applicable From" required hint="Only interactions recorded on or after this date will be transcribed by this rule.">
              <TextControl type="datetime-local" value={form.startDateTime} onChange={update("startDateTime")} icon={<CalendarDays size={15} />} />
            </Field>
...
export default function TranscriptionPage() {
  ...
  useEffect(() => {
    const fetchPrivileges = async () => {
      try {
        ...
        const orgId = typeof window !== "undefined" ? sessionStorage.getItem("selectedOrgId") : "";
        ...
        if (isSuper && privs.length === 0) {
          ...
          setPrivileges([{ PrivilegeId: 30 }, { PrivilegeId: 11 }, { PrivilegeId: 29 }]);
        } else {
          setPrivileges(privs);
        }
      } catch (err) {
    ...
    fetchPrivileges();
    // Commented out as these APIs are not used in the current process to avoid 500 errors
    // loadConfigs();
    // fetchDDL(
    //   "providers",
    //   "/api/integrationWorkspace/Transcription/STTEngineDDL",
    //   (json) => { setProviders(Array.isArray(json.providerList) ? json.providerList : []); setProvidersLoading(false); },
  ...
  const filteredConfigs = useMemo(() => {
    ...
    return allConfigs.filter((row) => {
      ...
      const instance = String(row.InstanceName ?? row.instanceName ?? "").toLowerCase();
  ...
  const openEdit = useCallback((row) => {
    ...
    const matchedProvider = providers.find((p) =>
      (p.Provider_name ?? p.providerName ?? p.name ?? "") === row.TranscriptionEngine,
    );
    const resolvedEngineId = String(matchedProvider?.Id ?? matchedProvider?.id ?? "");
    setForm({
      ...INITIAL_FORM,
      id: row.RuleId,
      ...
      processingPriority: row.ProcessingPriority ?? row.processingPriority ?? "normal",
      ...
      durationValueMax: String(row.DurationValueMax ?? row.durationValueMax ?? ""),
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        isEdit ? "/api/workFlow/updateTranscriptionRule" : "/api/workFlow/SaveTranscriptionRule",
  ...
  const getEngineName = useCallback((engineId) => {
    if (!engineId) return "—";
    const p = providers.find((p) => String(p.Id ?? p.id) === String(engineId));
    return p ? (p.Provider_name ?? p.providerName ?? String(engineId)) : String(engineId);
  }, [providers]);
  ...
  const hasAccess = privileges.some((p) => [30, 11, 29].includes(p.PrivilegeId));
  ...
  if (view === "form") return (
    <FormPage
      ...
      providers={providers} providersLoading={providersLoading}
      ...
      <div className="rounded-2xl border border-[var(--brand-input-border)] bg-card shadow-sm px-5 py-3.5 mb-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="m-0 text-sm font-bold text-foreground">Transcription Rules</p>
              <p className="m-0 text-xs text-muted-foreground mt-0.5">
                Manage rules to transcribe interactions using configurable engines and priorities.
              </p>
            </div>
          </div>
      ...
      <div className="rounded-2xl border border-[var(--brand-input-border)] bg-card shadow-sm overflow-hidden">
        ...
        <div className="flex items-center justify-between flex-wrap gap-2.5 px-5 py-3 border-b border-[var(--brand-input-border)] bg-[color-mix(in_srgb,var(--brand-primary)_4%,transparent)]">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] font-bold text-foreground">All Rules</span>
            {!configsLoading && (
                        ...
                        <div className="flex flex-col items-center gap-2.5">
                          {searchQuery ? (
                            ...
                            <>
                              ...
                              <p className="m-0 text-sm font-semibold">No transcription rules yet</p>
                              <button onClick={openAdd}
                    ...
                    pagedConfigs.map((row, idx) => (
                      <tr key={row.RuleId ?? idx}
                        ...
                        <td className="px-4 py-3">
                          <EngineBadge name={row.TranscriptionEngine ?? row.transcriptionEngine ?? "—"} />
                        </td>
                        <td className="px-4 py-3 font-medium text-foreground text-xs">
                          {row.InstanceName ?? row.instanceName ?? "—"}
                        </td>
                        <td className="px-4 py-3">
                          <PriorityBadge priority={row.ProcessingPriority ?? row.processingPriority} />
                        </td>
                        <td className="px-4 py-3 text-muted-foreground text-xs whitespace-nowrap">
                          {fmtDate(row.StartDateTime ?? row.startDateTime)}
                        </td>
                        <td className="px-4 py-3">
                          <StatusBadge enabled={!!(row.RuleIsEnabled ?? row.ruleIsEnabled)} />
                        </td>
                        <td className="px-4 py-3">
