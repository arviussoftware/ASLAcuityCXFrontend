// components/instanceNameDDL.jsx
...
// Fetch instance names from API
const fetchInstanceNames = async () => {
  ...
  const response = await fetch("/api/instanceNameDDL", {
    method: "GET",
    headers: {
  ...
  if (!response.ok) {
    let message = `Failed to fetch instance names (HTTP ${response.status})`;
    try {
  ...
  if (result.success) {
    ...
    throw new Error(result.message || "Failed to fetch instance names");
  }
};
...
const InstanceNameDDL = ({
  ...
  useEffect(() => {
    const loadOptions = async () => {
      try {
        const instanceList = await fetchInstanceNames();
        const formattedOptions = instanceList.map((instance) => ({
          value: `${instance.appid}|${instance.PlatformId}`, // unique key combining both
        ...
        console.error("Error loading instance name options:", error);
      }
    };
...
export default withAuth(InstanceNameDDL);
